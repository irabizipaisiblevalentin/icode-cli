...
V2 core discovers config documents named `icode.json` or `icode.jsonc` in the global config directory, ancestor project directories, and `.icode` config directories. The legacy `config.json` filename is not supported in V2.
...
The configured `plugins` list represents package-loaded plugins only. Local plugin code remains discovered from plugin directories such as `.icode/plugins/`; v2 does not port arbitrary configured local paths or file URLs into this field.
...
Policy evaluation will consume authored config documents in reverse order while preserving statement order inside each document. The precedence of `.icode` policy sources remains open until `.icode` configuration is reviewed.
