      ...
      Effect.gen(function* () {
        return Config.of(
          yield* EffectConfig.all({
            ...
            username: EffectConfig.string("OPENCODE_SERVER_USERNAME").pipe(EffectConfig.withDefault("icode")),
          }),
        )
      }),
    )
...
export function header(credentials?: Credentials) {
  ...
  return `Basic ${Buffer.from(`${credentials?.username ?? process.env.OPENCODE_SERVER_USERNAME ?? "icode"}:${password}`).toString("base64")}`
}
