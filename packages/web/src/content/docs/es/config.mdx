...
2. **Configuración global** (`~/.config/opencode/icode.json`) - preferencias del usuario
3. **Configuración personalizada** (`OPENCODE_CONFIG` env var): anulaciones personalizadas
4. **Configuración del proyecto** (`icode.json` en el proyecto): configuración específica del proyecto
5. Directorios **`.icode`** - agentes, comandos, complementos
...
:::note
Los directorios `.icode` y `~/.config/opencode` usan **nombres en plural** para los subdirectorios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` y `themes/`. También se admiten nombres singulares (por ejemplo, `agent/`) para compatibilidad con versiones anteriores.
:::
...
Coloque su configuración global OpenCode en `~/.config/opencode/icode.json`. Utilice la configuración global para las preferencias de todo el usuario, como temas, proveedores o combinaciones de teclas.
...
Agregue `icode.json` en la raíz de su proyecto. La configuración del proyecto tiene la mayor prioridad entre los archivos de configuración estándar: anula las configuraciones globales y remotas.
...
modos y complementos como el directorio estándar `.icode`, y debería
...
El directorio personalizado se carga después de los directorios global config y `.icode`, por lo que **puede anular** su configuración.
...
También puede definir agentes utilizando archivos de Markdown en `~/.config/opencode/agents/` o `.icode/agents/`. [Más información aquí](/docs/agents).
...
También puede definir comandos utilizando archivos de Markdown en `~/.config/opencode/commands/` o `.icode/commands/`. [Más información aquí](/docs/commands).
...
Coloque los archivos de complemento en `.icode/plugins/` o `~/.config/opencode/plugins/`. También puedes cargar complementos desde npm a través de la opción `plugin`.
