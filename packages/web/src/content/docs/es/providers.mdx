...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Para una configuración persistente o específica del proyecto, utilice `icode.json`:
...
Archivo `icode.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Personalizar a través de `icode.json`:
...
4. Cree o actualice su archivo `icode.json` en el directorio de su proyecto:
