...
- **macOS/Linux**: `~/.config/opencode/icode.jsonc` (o `~/.config/opencode/icode.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/opencode/icode.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\opencode\icode.jsonc`
...
Si su `icode.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.
