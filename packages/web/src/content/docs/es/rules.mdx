...
Puede especificar archivos de instrucciones personalizados en su `icode.json` o el `~/.config/opencode/icode.json` global. Esto le permite a usted y a su equipo reutilizar las reglas existentes en lugar de tener que duplicarlas en AGENTS.md.
...
El enfoque recomendado es utilizar el campo `instructions` en `icode.json`:
...
:::tip
Para monorepos o proyectos con estándares compartidos, usar `icode.json` con patrones globales (como `packages/*/AGENTS.md`) es más fácil de mantener que las instrucciones manuales.
:::
