...
Du kan spesifisere egendefinerte instruksjonsfiler i din `icode.json` eller den globale `~/.config/opencode/icode.json`. Dette lar deg og teamet ditt gjenbruge eksisterende regler i stedet for at måtte duplisere dem til AGENTS.md.
...
Den anbefalte tilnærmingen er at bruge feltet `instructions` i `icode.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er brug av `icode.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
