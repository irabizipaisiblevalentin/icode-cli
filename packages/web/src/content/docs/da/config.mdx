...
2. **Global config** (`~/.config/opencode/icode.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`OPENCODE_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`icode.json` i projekt) - projektspecifikke indstillinger
5. **`.icode` mapper** - agent, kommandoer, plugins
...
:::note
`.icode` og `~/.config/opencode` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Placer din globale OpenCode-konfiguration i `~/.config/opencode/icode.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `icode.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.
...
modes og plugins ligesom standard `.icode` mappen, og bør
...
Den brugerdefinerede mappe indlæses efter den globale konfig og `.icode` mapper, så den **kan tilsidesætte** deres indstillinger.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opencode/agents/` eller `.icode/agents/`. [Learn more here](/docs/agents).
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opencode/commands/` eller `.icode/commands/`. [Learn more here](/docs/commands).
...
Placer plugin-filer i `.icode/plugins/` eller `~/.config/opencode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
