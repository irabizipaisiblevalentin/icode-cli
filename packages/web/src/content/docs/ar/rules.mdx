...
يمكنك تحديد ملفات تعليمات مخصصة في `icode.json` أو في الملف العام `~/.config/opencode/icode.json`. يتيح ذلك لك ولفريقك إعادة استخدام القواعد الموجودة بدلًا من تكرارها داخل AGENTS.md.
...
النهج الموصى به هو استخدام الحقل `instructions` في `icode.json`:
...
:::tip
بالنسبة للمستودعات متعددة الحزم (monorepos) أو المشاريع ذات المعايير المشتركة، فإن استخدام `icode.json` مع أنماط glob (مثل `packages/*/AGENTS.md`) أسهل للصيانة من التعليمات اليدوية.
:::
