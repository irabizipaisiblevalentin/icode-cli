...
You can specify custom instruction files in your `icode.json` or the global `~/.config/opencode/icode.json`. This allows you and your team to reuse existing rules rather than having to duplicate them to AGENTS.md.
...
The recommended approach is to use the `instructions` field in `icode.json`:
...
:::tip
For monorepos or projects with shared standards, using `icode.json` with glob patterns (like `packages/*/AGENTS.md`) is more maintainable than manual instructions.
:::
