...
- **macOS/Linux**: `~/.config/opencode/icode.jsonc` (または `~/.config/opencode/icode.json`)
- **macOS/Linux** (古いインストール): `~/.local/share/opencode/icode.jsonc`
- **Windows**: `WIN+R` を押して `%USERPROFILE%\.config\opencode\icode.jsonc` を貼り付けます
...
`icode.json(c)` に `server` セクションが含まれている場合は、それを一時的に削除し、デスクトップアプリを再起動します。
