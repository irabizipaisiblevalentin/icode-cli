...
- 專案設定：`.icode/skills/<name>/SKILL.md`
...
在此過程中，它會載入 `.icode/` 中所有匹配的 `skills/*/SKILL.md`，以及匹配的 `.claude/skills/*/SKILL.md` 或 `.agents/skills/*/SKILL.md`。
...
建立 `.icode/skills/git-release/SKILL.md`，內容如下：
...
在 `icode.json` 中使用基於模式的權限來控制代理可以存取哪些技能：
