...
- **macOS/Linux**: `~/.config/opencode/icode.jsonc`（或 `~/.config/opencode/icode.json`）
- **macOS/Linux**（舊版安裝）: `~/.local/share/opencode/icode.jsonc`
- **Windows**: 按 `WIN+R` 並貼上 `%USERPROFILE%\.config\opencode\icode.jsonc`
...
如果你的 `icode.json(c)` 包含 `server` 部分，請暫時移除該部分並重新啟動桌面應用程式。
