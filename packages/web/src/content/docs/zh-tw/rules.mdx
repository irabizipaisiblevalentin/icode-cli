...
您可以在 `icode.json` 或全域設定檔 `~/.config/opencode/icode.json` 中指定自訂指令檔案。這允許您和團隊複用現有規則，而無需將它們複製到 AGENTS.md 中。
...
建議的方式是使用 `icode.json` 中的 `instructions` 欄位：
...
:::tip
對於 monorepo 或具有共享標準的專案，使用 `icode.json` 搭配 glob 模式（如 `packages/*/AGENTS.md`）比手動指定指令更易於維護。
:::
