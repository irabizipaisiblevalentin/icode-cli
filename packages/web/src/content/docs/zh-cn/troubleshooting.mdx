...
- **macOS/Linux**（旧版安装）: `~/.local/share/opencode/icode.jsonc`
- **Windows**: 按 `WIN+R` 并粘贴 `%USERPROFILE%\.config\opencode\icode.jsonc`
...
如果你的 `icode.json(c)` 包含 `server` 部分，请临时移除该部分并重新启动桌面应用。
