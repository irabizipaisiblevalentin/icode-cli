...
Możesz określić niestandardowe pliki instrukcji w swoim `icode.json` lub globalnym `~/.config/opencode/icode.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
Zalecanym podejściem jest użycie pola `instructions` w `icode.json`:
...
:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `icode.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
