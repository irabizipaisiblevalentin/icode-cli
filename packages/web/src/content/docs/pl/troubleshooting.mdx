...
- **macOS/Linux**: `~/.config/opencode/icode.jsonc` (lub `~/.config/opencode/icode.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/icode.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\opencode\icode.jsonc`
...
Jeśli Twój `icode.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.
