...
- Project config: `.icode/skills/<name>/SKILL.md`
...
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.icode/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.
...
Utwórz `.icode/skills/git-release/SKILL.md` w ten sposób:
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `icode.json`:
...
**Dla agentów wbudowanych** (w `icode.json`):
