...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `icode.json`:
...
`icode.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
4. Utwórz lub zaktualizuj plik `icode.json` w katalogu projektu:
