...
- **macOS/Linux**: `~/.config/opencode/icode.jsonc` (or `~/.config/opencode/icode.json`)
- **macOS/Linux** (older installs): `~/.local/share/opencode/icode.jsonc`
- **Windows**: Press `WIN+R` and paste `%USERPROFILE%\.config\opencode\icode.jsonc`
...
If your `icode.json(c)` contains a `server` section, temporarily remove it and restart the desktop app.
