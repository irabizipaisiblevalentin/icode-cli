...
- **macOS/Linux**: `~/.config/opencode/icode.jsonc` (ili `~/.config/opencode/icode.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencode/icode.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\opencode\icode.jsonc`
...
Ako vaš `icode.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.
