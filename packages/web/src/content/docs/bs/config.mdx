...
2. **Globalna konfiguracija** (`~/.config/opencode/icode.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`OPENCODE_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`icode.json` u projektu) - postavke specifične za projekat
5. **`.icode` direktoriji** - agenti, komande, dodaci
...
:::note
Direktoriji `.icode` i `~/.config/opencode` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Postavite svoju globalnu OpenCode konfiguraciju u `~/.config/opencode/icode.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Dodajte `icode.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `OPENCODE_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.icode` direktorij, i trebali bi pratiti istu strukturu.
...
Prilagođeni direktorij se učitava nakon direktorija globalne konfiguracije i `.icode`, tako da **može nadjačati** njihove postavke.
...
Stari `theme`, `keybinds`, i `tui` ključevi u `icode.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/opencode/agents/` ili `.icode/agents/`. [Saznajte više ovdje](/docs/agents).
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/opencode/commands/` ili `.icode/commands/`. [Saznajte više ovdje](/docs/commands).
...
Postavite datoteke dodataka u `.icode/plugins/` ili `~/.config/opencode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
