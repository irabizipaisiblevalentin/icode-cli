...
Mozete navesti prilagodene datoteke uputstava u `icode.json` ili globalnom `~/.config/opencode/icode.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
### Korištenje icode.json

Preporuceni pristup je da koristite `instructions` polje u `icode.json`:
...
:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `icode.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
