...
Du kan spesifisere egendefinerte instruksjonsfiler i din `icode.json` eller den globale `~/.config/opencode/icode.json`. Dette lar deg og teamet ditt gjenbruke eksisterende regler i stedet for å måtte duplisere dem til AGENTS.md.
...
Den anbefalte tilnærmingen er å bruke feltet `instructions` i `icode.json`:
...
:::tip
For monorepos eller prosjekter med delte standarder er bruk av `icode.json` med glob-mønstre (som `packages/*/AGENTS.md`) mer vedlikeholdbart enn manuelle instruksjoner.
:::
