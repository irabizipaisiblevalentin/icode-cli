...
- Lokalt ved å plassere dem i `.icode/tools/`-katalogen til prosjektet ditt.
...
```ts title=".icode/tools/database.ts" {1}
...
```ts title=".icode/tools/project.ts" {8}
import { tool } from "@opencode-ai/plugin"
...
```python title=".icode/tools/add.py"
import sys
...
```ts title=".icode/tools/python-add.ts" {10}
import { tool } from "@opencode-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".icode/tools/add.py")
