...
2. **Global konfigurasjon** (`~/.config/opencode/icode.json`) - brukerinnstillinger
3. **Egendefinert konfigurasjon** (`OPENCODE_CONFIG` env var) - egendefinerte overstyringer
4. **Prosjektkonfigurasjon** (`icode.json` i prosjekt) - prosjektspesifikke innstillinger
5. **`.icode` kataloger** - agenter, kommandoer, plugins
...
:::note
`.icode` og `~/.config/opencode` katalogene bruker **flertallsnavn** på underkataloger: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkeltnavn (f.eks. `agent/`) støttes også for bakoverkompatibilitet.
:::
...
Plasser din globale OpenCode-konfigurasjon i `~/.config/opencode/icode.json`. Bruk global konfigurasjon for brukerspesifikke preferanser som temaer, leverandører eller nøkkelbindinger.
...
Legg til `icode.json` i prosjektroten din. Prosjektkonfigurasjon har den høyeste prioritet blant standard config-filer - den overstyrer både globale og eksterne konfigurasjoner.
...
moduser og plugins akkurat som standard `.icode` katalog, og bør
...
Den egendefinerte katalogen lastes inn etter den globale konfigurasjonen og `.icode`-katalogene, slik at den **kan overstyre** innstillingene deres.
...
Du kan også definere agenter ved å bruke markdown-filer i `~/.config/opencode/agents/` eller `.icode/agents/`. [Les mer her](/docs/agents).
...
Du kan også definere kommandoer ved å bruke markdown-filer i `~/.config/opencode/commands/` eller `.icode/commands/`. [Les mer her](/docs/commands).
...
Plasser plugin-filer i `.icode/plugins/` eller `~/.config/opencode/plugins/`. Du kan også laste inn plugins fra npm gjennom alternativet `plugin`.
