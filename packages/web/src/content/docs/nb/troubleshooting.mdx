...
- **macOS/Linux**: `~/.config/opencode/icode.jsonc` (eller `~/.config/opencode/icode.json`)
- **macOS/Linux** (eldre installasjoner): `~/.local/share/opencode/icode.jsonc`
- **Windows**: Trykk `WIN+R` og lim inn `%USERPROFILE%\.config\opencode\icode.jsonc`
...
Hvis `icode.json(c)` inneholder en `server`-del, fjern den midlertidig og start skrivebordsappen på nytt.
