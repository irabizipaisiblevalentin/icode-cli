...
2. **Config globale** (`~/.config/opencode/icode.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `OPENCODE_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`icode.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.icode`** - agenti, comandi, plugin
...
:::note
Le directory `.icode` e `~/.config/opencode` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Metti la configurazione globale di OpenCode in `~/.config/opencode/icode.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `icode.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `OPENCODE_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.icode` e dovrebbe seguire la stessa struttura.
...
La directory personalizzata viene caricata dopo la configurazione globale e le directory `.icode`, quindi **puo sovrascrivere** le loro impostazioni.
...
Le chiavi legacy `theme`, `keybinds` e `tui` in `icode.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.icode/agents/`. [Scopri di piu](/docs/agents).
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.icode/commands/`. [Scopri di piu](/docs/commands).
...
Metti i file dei plugin in `.icode/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
