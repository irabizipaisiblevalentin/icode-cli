...
- **macOS/Linux** (installazioni vecchie): `~/.local/share/opencode/icode.jsonc`
- **Windows**: premi `WIN+R` e incolla `%USERPROFILE%\.config\opencode\icode.jsonc`
...
Se il tuo `icode.json(c)` contiene una sezione `server`, rimuovila temporaneamente e riavvia l'app desktop.
