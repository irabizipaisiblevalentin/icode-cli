...
Puoi specificare file di istruzioni personalizzati nel tuo `icode.json` o nel globale `~/.config/opencode/icode.json`. Questo permette a te e al tuo team di riusare regole esistenti invece di doverle duplicare in AGENTS.md.
...
L'approccio consigliato e' usare il campo `instructions` in `icode.json`:
...
:::tip
Per monorepo o progetti con standard condivisi, usare `icode.json` con pattern glob (come `packages/*/AGENTS.md`) e' piu' manutenibile rispetto alle istruzioni manuali.
:::
