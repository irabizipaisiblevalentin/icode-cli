...
Pour une configuration spécifique au projet ou persistante, utilisez `icode.json` :
...
Fichier `icode.json`. Il est également recommandé de désactiver le partage de session.
...
4. Créez ou mettez à jour votre fichier `icode.json` dans le répertoire de votre projet :
