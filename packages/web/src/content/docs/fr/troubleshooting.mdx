...
- **macOS/Linux** : `~/.config/opencode/icode.jsonc` (ou `~/.config/opencode/icode.json`)
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencode/icode.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\opencode\icode.jsonc`
...
Si votre `icode.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.
