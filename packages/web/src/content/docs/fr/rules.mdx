...
Vous pouvez spécifier des fichiers d'instructions personnalisés dans votre `icode.json` ou dans le `~/.config/opencode/icode.json` global. Cela vous permet, à vous et à votre équipe, de réutiliser les règles existantes plutôt que d'avoir à les dupliquer dans AGENTS.md.
...
### Utilisation de icode.json

L'approche recommandée consiste à utiliser le champ `instructions` dans `icode.json` :
...
:::tip
Pour les monorepos ou les projets avec des normes partagées, l'utilisation de `icode.json` avec des modèles globaux (comme `packages/*/AGENTS.md`) est plus maintenable que les instructions manuelles.
:::
