...
It loads any matching `skills/*/SKILL.md` in `.icode/` and any matching `.claude/skills/*/SKILL.md` or `.agents/skills/*/SKILL.md` along the way.
...
Create `.icode/skills/git-release/SKILL.md` like this:
...
Control which skills agents can access using pattern-based permissions in `icode.json`:
...
**For built-in agents** (in `icode.json`):
