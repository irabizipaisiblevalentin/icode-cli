...
Özel talimat dosyalarını `icode.json` veya global `~/.config/opencode/icode.json`'de belirtebilirsiniz. Bu, sizin ve ekibinizin, mevcut kuralları AGENTS.md'ye kopyalamak yerine yeniden kullanmasına olanak tanır.
...
### icode.json'u kullanma

Önerilen yaklaşım, `instructions`'deki `icode.json` alanını kullanmaktır:
...
:::tip
Monorepo'lar veya ortak standartlara sahip projeler için, `icode.json`'yi glob desenleriyle (`packages/*/AGENTS.md` gibi) kullanmak, manuel talimatlara göre daha sürdürülebilirdir.
:::
