...
- **macOS/Linux**: `~/.config/opencode/icode.jsonc` (veya `~/.config/opencode/icode.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opencode/icode.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\opencode\icode.jsonc` yapistirin
...
`icode.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.
