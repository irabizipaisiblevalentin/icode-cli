...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   Projeye özel veya kalıcı yapılandırma için `icode.json` kullanın:
...
`icode.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
`icode.json` üzerinden özelleştirin:
...
4. Proje dizininizde `icode.json` dosyanızı oluşturun veya güncelleyin:
