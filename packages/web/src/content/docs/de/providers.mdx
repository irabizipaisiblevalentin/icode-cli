...
2. **Konfigurieren Sie die Authentifizierung** mit einer der folgenden Methoden:
   ...
   Für eine projektspezifische oder dauerhafte Konfiguration verwenden Sie `icode.json`:
...
`icode.json`-Datei. Es wird außerdem empfohlen, die Sitzungsfreigabe zu deaktivieren.
...
4. Erstellen oder aktualisieren Sie Ihre `icode.json`-Datei in Ihrem Projektverzeichnis:
