...
Du kannst zusaetzliche Anweisungsdateien in deiner `icode.json` oder in `~/.config/opencode/icode.json` angeben.
...
Empfohlen ist das `instructions`-Feld in der `icode.json`:
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `icode.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
