...
- **macOS/Linux**: `~/.config/opencode/icode.jsonc` (oder `~/.config/opencode/icode.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/icode.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\opencode\icode.jsonc`
...
Wenn deine `icode.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.
