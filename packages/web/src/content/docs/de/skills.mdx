...
Dabei werden passende `skills/*/SKILL.md` in `.icode/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.
...
Erstelle `.icode/skills/git-release/SKILL.md` zum Beispiel so:
...
Steuere in `icode.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `icode.json`):
