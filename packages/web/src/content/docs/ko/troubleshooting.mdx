...
-**macOS/리눅스**: `~/.config/opencode/icode.jsonc` (또는 `~/.config/opencode/icode.json`) -**macOS/Linux** (외부 설치): `~/.local/share/opencode/icode.jsonc`

- **Windows**: 압박 `WIN+R`와 풀 `%USERPROFILE%\.config\opencode\icode.jsonc`
...
`icode.json(c)`가 `server` 섹션을 포함하면 일시적으로 제거하고 데스크톱 앱을 다시 시작합니다.
