...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `icode.json`:
...
arquivo `icode.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Personalize através do `icode.json`:
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no icode.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `icode.json` no diretório do seu projeto:
