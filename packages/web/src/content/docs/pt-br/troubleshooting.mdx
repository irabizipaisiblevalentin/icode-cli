...
- **macOS/Linux** (instalações mais antigas): `~/.local/share/opencode/icode.jsonc`
- **Windows**: Pressione `WIN+R` e cole `%USERPROFILE%\.config\opencode\icode.jsonc`
...
Se seu `icode.json(c)` contiver uma seção `server`, remova-a temporariamente e reinicie o aplicativo de desktop.
