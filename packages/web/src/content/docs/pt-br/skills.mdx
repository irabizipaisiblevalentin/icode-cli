...
- Configuração do projeto: `.icode/skills/<name>/SKILL.md`
...
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.icode/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.
...
Crie `.icode/skills/git-release/SKILL.md` assim:
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `icode.json`:
...
**Para agentes embutidos** (em `icode.json`):
