...
Você pode especificar arquivos de instrução personalizados no seu `icode.json` ou no global `~/.config/opencode/icode.json`. Isso permite que você e sua equipe reutilizem regras existentes em vez de ter que duplicá-las no AGENTS.md.
...
A abordagem recomendada é usar o campo `instructions` em `icode.json`:
...
:::tip
Para monorepos ou projetos com padrões compartilhados, usar `icode.json` com padrões glob (como `packages/*/AGENTS.md`) é mais sustentável do que instruções manuais.
:::
