...
2. **Configuração global** (`~/.config/opencode/icode.json`) - preferências do usuário
3. **Configuração personalizada** (`OPENCODE_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`icode.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.icode`** - agentes, comandos, plugins
...
:::note
Os diretórios `.icode` e `~/.config/opencode` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
Coloque sua configuração global do opencode em `~/.config/opencode/icode.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Adicione `icode.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `OPENCODE_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.icode`, e deve seguir a mesma estrutura.
...
O diretório personalizado é carregado após a configuração global e os diretórios `.icode`, então ele **pode substituir** suas configurações.
...
Chaves legadas `theme`, `keybinds` e `tui` em `icode.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.icode/agents/`. [Saiba mais aqui](/docs/agents).
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.icode/commands/`. [Saiba mais aqui](/docs/commands).
...
Coloque arquivos de plugin em `.icode/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
