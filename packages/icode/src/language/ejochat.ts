...
export interface EjoChatOptions {
  ...
  model: string
  apiKey: string
  /** Optional system prompt. Never includes secrets. */
  system?: string
  maxTokens?: number
  signal?: AbortSignal
}
 ...
 * All text sent here passes through token protection and secret redaction
 * upstream, and the API key is only ever sent in the auth headers.
 */
export async function chat(options: EjoChatOptions, messages: EjoChatMessage[]): Promise<EjoChatResult> {
  ...
  if (!res.ok) {
    let detail = ""
    try {
      const copied = await res.clone().text()
      detail = redactSecrets(copied).slice(0, 300)
    } catch {
      detail = ""
    }
    throw new EjoChatError(`EjoChat API error ${res.status}`, res.status, detail)
  }
...
function extractText(data: Record<string, unknown>): string {
  // Common response shapes, tried in order of likelihood.
...
/** True if EjoChat should be called for this request (config + intent signals). */
export function shouldUseEjoChat(config: ICodeConfig, needsRw: boolean): boolean {
  return config.ejochatEnabled && needsRw
}
