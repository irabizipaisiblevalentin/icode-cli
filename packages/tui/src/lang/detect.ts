import { KINYARWANDA_MARKERS, KINYARWANDA_WORDS, INTENT_VERBS } from "./words"
...
export const LANGUAGE_MODES: LanguageMode[] = ["rw", "auto", "en"]
...
export function extractIntent(text: string): BaseIntent {
  ...
  const pathMatch = /[\w.@~/-]+\.(?:ts|tsx|js|jsx|py|json|jsonc|toml|yaml|yml|md|rs|go|c|cpp|java|rb|php|sh|css|html|txt|sql|log|lock|mod|sum)\b/i.exec(
    text,
  )
  if (pathMatch) intent.target = pathMatch[0]

  if (
    /(idakora|ntago[ _-]?akora|ntakora|yanze|anze|itangiye|ikosa|amakosa|bug|error|failed|permission denied|crash)/i.test(
      lower,
    )
  ) {
  ...
  intent.question =
    /\?$/.test(text.trim()) || /^(niba|mbwira|sobanura|kuki|kubera(?:iki)?|ni\s+iki|ni\s+iyi|niibiki)\b/i.test(lower)
