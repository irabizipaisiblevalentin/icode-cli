    ...
    <Switch>
      <Match when={!props.done}>
        <box flexDirection="row">
          <Spinner color={fg()}>{props.title ? `${t("thinking")}: ${props.title}` : t("thinking")}</Spinner>
        </box>
      </Match>
      <Match when={true}>
...
function GenericTool(props: ToolProps) {
  ...
  return (
    <Show
      ...
      fallback={
        <InlineTool icon="⚙" pending={t("writingCommand")} complete={true} part={props.part}>
...
function Shell(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="$" pending={t("writingCommand")} complete={stringValue(props.input.command)} part={props.part}>
  ...
  return (
    <Switch>
      <Match when={props.metadata.diagnostics !== undefined}>
        <BlockTool title={"# " + t("wrote") + " " + pathFormatter.format(stringValue(props.input.filePath))} part={props.part}>
...
function Glob(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending={t("findingFiles")} complete={stringValue(props.input.pattern)} part={props.part}>
    ...
    <>
      ...
      <For each={loaded()}>
        {(filepath) => (
          <box paddingLeft={3}>
            <text paddingLeft={3} fg={theme.textMuted}>
              ↳ {t("loadedFile")} {pathFormatter.format(filepath)}
            </text>
          </box>
        )}
      </For>
...
function Grep(props: ToolProps) {
  const pathFormatter = usePathFormatter()
  return (
    <InlineTool icon="✱" pending={t("searchingContent")} complete={stringValue(props.input.pattern)} part={props.part}>
...
function WebFetch(props: ToolProps) {
  return (
    <InlineTool icon="%" pending={t("fetchingWeb")} complete={stringValue(props.input.url)} part={props.part}>
...
function WebSearch(props: ToolProps) {
  return (
    <InlineTool icon="◈" pending={t("searchingWeb")} complete={stringValue(props.input.query)} part={props.part}>
...
function Edit(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="←" pending={t("preparingEdit")} complete={stringValue(props.input.filePath)} part={props.part}>
...
function ApplyPatch(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="%" pending={t("preparingPatch")} failure="Patch failed" complete={false} part={props.part}>
          Patch
        </InlineTool>
      </Match>
    </Switch>
...
function Question(props: ToolProps) {
  ...
  return (
    <Switch>
      ...
      <Match when={true}>
        <InlineTool icon="→" pending={t("askingQuestions")} complete={count()} part={props.part}>
...
function Skill(props: ToolProps) {
  return (
    <InlineTool icon="→" pending={t("loadingSkill")} complete={stringValue(props.input.name)} part={props.part}>
