...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "icode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "icode.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "icode.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "icode.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["icode.json", "icode.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })
      ...
      expect(result).toEqual([
        ...
        path.join(project, "icode.json"),
        path.join(project, "icode.jsonc"),
      ])
    })
  })
