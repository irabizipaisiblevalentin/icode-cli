...
function globalConfigFile() {
  const candidates = ["icode.jsonc", "icode.json", "config.json"].map((file) =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "icode.json"), env))
      result = mergeConfig(result, yield* loadFile(path.join(Global.Path.config, "icode.jsonc"), env))
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("icode", ctx.directory, ctx.worktree).pipe(Effect.orDie)) {
        ...
        for (const dir of directories) {
          if (dir.endsWith(".icode") || dir === Flag.OPENCODE_CONFIG_DIR) {
            for (const file of ["icode.json", "icode.jsonc"]) {
          ...
          // Auto-discovered plugins under `.icode/plugin(s)` are already local files, so ConfigPlugin.load
