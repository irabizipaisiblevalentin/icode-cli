...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, icode does not support MCP authentication yet.`
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`icode models\` to list available models`,
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`icode auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
