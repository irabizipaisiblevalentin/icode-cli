...
// CLI entry point for `icode run` and `icode --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`icode --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`icode --mini --attach`): connects to a running
//      icode server and runs interactive mode against it.
//
...
type PromptPart = FilePart | { type: "text"; text: string }
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run icode with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running icode server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
      ...
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to OPENCODE_SERVER_USERNAME or 'icode')",
      })
      .option("dir", {
        type: "string",
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    const { runPasscodeGate, startHeartbeat } = yield* Effect.promise(() => import("@/passcode/client"))
