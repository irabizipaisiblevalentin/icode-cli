...
export function RunPromptBody(props: {
  ...
  return (
    <box width="100%">
      <box paddingTop={1} paddingBottom={1} paddingRight={2} flexDirection="row" width="100%">
        <text flexShrink={0} fg={props.theme().muted} paddingRight={1}>
          ⚡ iCode &gt;
        </text>
        <textarea
          width="100%"
...
export function createPromptState(input: PromptInput): PromptState {
  ...
  const slashOptions = createMemo<SlashOption[]>(() => {
    const builtins = [
      ...
      { kind: "slash", name: "exit", display: "/exit", description: "close iCode" } satisfies SlashOption,
    ]
