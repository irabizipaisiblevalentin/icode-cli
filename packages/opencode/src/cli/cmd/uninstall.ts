...
export const UninstallCommand = {
  command: "uninstall",
  describe: "uninstall icode and remove all related files",
  builder: (yargs: Argv) =>
    yargs
      .option("keep-config", {
        alias: "c",
...
async function showRemovalSummary(targets: RemovalTargets, method: Installation.Method) {
  ...
  if (method !== "curl" && method !== "unknown") {
    const cmds: Record<string, string> = {
      npm: "npm uninstall -g icode-ai",
      pnpm: "pnpm uninstall -g icode-ai",
      bun: "bun remove -g icode-ai",
      yarn: "yarn global remove icode-ai",
      brew: "brew uninstall icode",
      choco: "choco uninstall icode",
      scoop: "scoop uninstall icode",
    }
...
async function executeUninstall(method: Installation.Method, targets: RemovalTargets) {
  ...
  if (method !== "curl" && method !== "unknown") {
    ...
    if (cmd) {
      ...
      const result = await Process.run(method === "choco" ? ["choco", "uninstall", "icode", "-y", "-r"] : cmd, {
        nothrow: true,
      })
      if (result.code !== 0) {
...
async function getShellConfigFile(): Promise<string | null> {
  ...
  for (const file of candidates) {
    ...
    if (content.includes("# icode") || content.includes(".icode/bin")) {
      return file
    }
  }
...
async function cleanShellConfig(file: string) {
  ...
  for (const line of lines) {
    ...
    if (skip) {
      skip = false
      if (trimmed.includes(".icode/bin") || trimmed.includes("fish_add_path")) {
        continue
      }
    }

    if (
      (trimmed.startsWith("export PATH=") && trimmed.includes(".icode/bin")) ||
      (trimmed.startsWith("fish_add_path") && trimmed.includes(".icode"))
    ) {
      continue
    }
