...
const WORKFLOW_FILE = ".github/workflows/icode.yml"
...
jobs:
  icode:
    if: |
      ...
      contains(github.event.comment.body, ' /icode') ||
      startsWith(github.event.comment.body, '/icode')
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    try {
      ...
      console.log("icode session", session.id)
    ...
    async function getUserPrompt() {
      ...
      const mentions = (process.env["MENTIONS"] || "/icode,/oc")
        .split(",")
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to icode...")
    ...
    function generateBranchName(type: "issue" | "pr" | "schedule" | "dispatch") {
      ...
      if (type === "schedule" || type === "dispatch") {
        ...
        return `icode/${type}-${hex}-${timestamp}`
      }
      return `icode/${type}${issueId}-${timestamp}`
    }
    ...
    function footer(opts?: { image?: boolean }) {
      ...
      const shareUrl = shareId ? `[icode session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
