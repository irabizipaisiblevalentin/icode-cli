...
- If `engines.icode` is absent, no check is performed (backward compatible).
...
- For local scope, path is `<worktree>/.icode` only when VCS is git and `worktree !== "/"`; otherwise `<directory>/.icode`.
- Root-worktree fallback (`worktree === "/"` uses `<directory>/.icode`) is covered by regression tests.
...
When `plugin` entries exist in a writable `.icode` dir or `OPENCODE_CONFIG_DIR`, OpenCode installs `@opencode-ai/plugin` into that dir and writes:
...
- Local plugins persist installed themes under the local `.icode/themes` area near the plugin config source.
...
- Local smoke plugin: `.icode/plugins/tui-smoke.tsx`
- Local vim plugin: `.icode/plugins/tui-vim.tsx`
- Local smoke config: `.icode/tui.json`
- Local smoke theme: `.icode/plugins/smoke-theme.json`
